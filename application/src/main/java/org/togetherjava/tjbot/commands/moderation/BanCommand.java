...
public final class BanCommand extends SlashCommandAdapter {
     ...
     *
     * @param database used to persist information for temporary bans, such as when to unban them
     *        again
     */
    ...
    private AuditableRestAction<Void> banUser(@NotNull User target, @NotNull Member author,
        ...
        if (temporaryBanData != null) {
            database.write(context -> {
                context.newRecord(TemporaryModerationActions.TEMPORARY_MODERATION_ACTIONS)
                    .setUserId(target.getIdLong())
                    .setGuildId(guild.getIdLong())
                    .setActionType(ModerationUtils.Action.BAN.name())
                    .setActionExpires(temporaryBanData.unbanTime)
                    .insert();
            });
        }
